...
export async function bankCommand(prompt, options) {
    try {
        printTitleBox();
...
async function generateResponse(prompt, instructions) {
    try {
...
async function secondCall(functionResponse, messages, toolCaller, tools) {
    if (!openai) {
        throw new Error("OpenAI client is not initialized");
    }
    ...
    const message = response2.choices && response2.choices[0] && response2.choices[0].message
...
async function toolCall(message, tools, messages) {
